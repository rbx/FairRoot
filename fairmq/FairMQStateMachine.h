  ...
  public:
    ...
    struct InitTaskFct
    {
        ...
        void operator()(EVT const&, FSM& fsm, SourceState&, TargetState&)
        {
            ...
            fsm.InitTaskWrapper();
            // fsm.fInitializingTaskThread = std::thread(&FairMQFSM::InitTaskWrapper, &fsm);
        }
    };
    ...
    bool CheckCurrentState(const std::string& state) const { return state == GetCurrentStateName(); }

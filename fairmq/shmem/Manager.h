...
class Manager
{
  ...
  public:
    ...
    boost::interprocess::mapped_region* CreateRegion(const size_t size, const uint64_t id);
    ...
    void SetRegionCallback(uint64_t, FairMQRegionCallback);
  ...
  private:
    ...
    std::unordered_map<uint64_t, FairMQRegionCallback> fRegionCallbacks; // TODO: better container?
    std::mutex fAckMtx;
    std::mutex fRegionControlMtx;
    std::condition_variable fRegionControlCV;
};

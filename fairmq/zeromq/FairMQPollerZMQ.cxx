    ...
    , fOffsetMap()
{
    ...
    {
        FairMQSocketZMQ const& socket = static_cast<FairMQSocketZMQ const&>(channels.at(i).GetSocket());

        fItems[i].socket = socket.Socket();
        fItems[i].fd = 0;
        fItems[i].revents = 0;

        int type = 0;
        size_t size = sizeof(type);
        zmq_getsockopt(socket.Socket(), ZMQ_TYPE, &type, &size);
    ...
    , fOffsetMap()
{
    ...
    try
    {
        ...
        for (auto& channel : channelList)
        {
            ...
            {
                FairMQSocketZMQ const& socket = static_cast<FairMQSocketZMQ const&>(channelsMap.at(channel).at(i).GetSocket());

                index = fOffsetMap[channel] + i;

                fItems[index].socket = socket.Socket();
                fItems[index].fd = 0;
    ...
    , fOffsetMap()
{
    ...
    fItems[0].socket = static_cast<const FairMQSocketZMQ&>(cmdSocket).Socket();
    fItems[0].fd = 0;
    fItems[0].events = ZMQ_POLLIN;
    fItems[0].revents = 0;

    fItems[1].socket = static_cast<const FairMQSocketZMQ&>(dataSocket).Socket();
    fItems[1].fd = 0;
    fItems[1].revents = 0;

    int type = 0;
    size_t size = sizeof(type);
    zmq_getsockopt(static_cast<const FairMQSocketZMQ&>(dataSocket).Socket(), ZMQ_TYPE, &type, &size);
